...
describe('Incoming Payment', function() {
  ...
  it('should record validated and prepared Incoming Payment', function(done){
    var incomingPayment = new IncomingPayment(incomingPaymentFixtures.incoming_payments.notification);
    incomingPayment._recordIncomingPayment(incomingPaymentFixtures.incoming_payments.to_record)
      .then(function(recodedPayment){
        chai.assert(recodedPayment.dataValues.transaction_hash, incomingPaymentFixtures.incoming_payments.valid.hash);
        chai.assert(recodedPayment.dataValues.transaction_state, incomingPaymentFixtures.incoming_payments.valid.result);
        done();
      })
      .error(function(error){
        done(error);
